...
describe('READONLY_ASSIST_INSTRUCTIONS', () => {
  it('requires getTaskContext, allows submitReviewPackage, and forbids draft writes', () => {
    ...
    expect(READONLY_ASSIST_INSTRUCTIONS).toContain('禁止根据预览')
    expect(READONLY_ASSIST_INSTRUCTIONS).toContain('发送后才建档解析')
    expect(READONLY_ASSIST_INSTRUCTIONS).toContain('只含本窗口发送后的档案摘要')
    expect(READONLY_ASSIST_INSTRUCTIONS).toContain('不要因为任务里可能有旧资料就去读全文')
