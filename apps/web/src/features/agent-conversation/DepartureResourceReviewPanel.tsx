...
export function DepartureResourceReviewPanel({
  departureId,
  conversationId,
  ...
  const packages = (collaborationQuery.data?.items ?? []).filter(
    (item) =>
      ...
      (item.confirmationUnit === DEPARTURE_RESOURCE_CONFIRMATION_UNIT ||
        item.payloadSchema === DEPARTURE_RESOURCE_REVIEW_PAYLOAD_SCHEMA),
  )
  if (packages.length === 0) {
    return null
  }
  return (
    <section className={styles.panel} aria-label="发团级资源审核">
      <div className={styles.heading}>
        <Typography.Text strong>发团级资源审核</Typography.Text>
        <Typography.Text type="secondary">
          全程或部分日期的整体费用按一条发团级资源确认；日期写入备注，不拆价，不自动提交应付。
        </Typography.Text>
      </div>
      {packages.map((pkg) => (
        <DepartureResourceReviewItem
          key={pkg.id}
          pkg={pkg}
...
function DepartureResourceReviewItem({
  pkg,
  departureId,
  canEdit,
  focused,
  ...
  const confirmMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      if (item?.status === 'succeeded') {
        message.success('已写入发团级资源，未提交应付')
      ...
      await queryClient.invalidateQueries({
        queryKey: ['departure-resources', departureId],
      })
    },
    onError: (error) => {
  ...
  const rejectMutation = useMutation({
    mutationFn: async () => {
      const expectedPackageVersion = await flushCorrections()
      return rejectAiReviewPackage('', pkg.id, {
    ...
    <article ref={rootRef} className={styles.item} data-review-package-id={pkg.id}>
        ...
        items={[
          {
            key: 'scope',
            label: '业务归属',
            children: <Typography.Text aria-label="发团级归属">发团级资源</Typography.Text>,
          },
        ]}
      />
      <Form layout="vertical" component="div" className={styles.fields}>
        ...
        <Form.Item label="备注" className={styles.field}>
          <Input.TextArea
            aria-label="备注候选"
            placeholder="覆盖部分日期时请写明服务日期，不拆价"
