...
describe('DepartureHeaderCard 时间信息', () => {
  it('将创建与更新时间收敛为次要元数据', () => {
    ...
    expect(screen.getByLabelText('执行班组')).toBeInTheDocument()
    expect(screen.getByText('司机名称')).toBeInTheDocument()
    expect(screen.getByText('导游名称')).toBeInTheDocument()
    expect(screen.getByText('司机车牌')).toBeInTheDocument()
  })
})
