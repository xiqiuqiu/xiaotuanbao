...
function renderHarness(onReady: (api: ReturnType<typeof useSourceOrdersTabMutations>) => void) {
...
describe('useSourceOrdersTabMutations save and generate', () => {
  ...
  it('creates the source order then generates receivables', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('已保存并提交应收')).toBeInTheDocument()
    })
  })
  ...
  it('warns when save succeeds but generate fails', async () => {
    ...
    await waitFor(() => {
      ...
      expect(generateReceivables).toHaveBeenCalledWith('order-new')
      expect(
        screen.getByText('客源单已保存，但提交应收失败：路径金额须大于 0'),
      ).toBeInTheDocument()
    })
  })
})
