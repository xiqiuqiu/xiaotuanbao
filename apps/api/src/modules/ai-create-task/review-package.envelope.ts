...
function capabilityRefForReviewPackage(confirmationUnit: string) {
  ...
  if (confirmationUnit === DEPARTURE_RESOURCE_CONFIRMATION_UNIT) {
    return DEPARTURE_DEPARTURE_RESOURCE_PROPOSE_CAPABILITY_REF
  }
...
function payloadSchemaForConfirmationUnit(confirmationUnit: string) {
  ...
  if (confirmationUnit === DEPARTURE_RESOURCE_CONFIRMATION_UNIT) {
    return DEPARTURE_RESOURCE_REVIEW_PAYLOAD_SCHEMA
  }
