...
test.describe('agent conversation modes #370', () => {
  test('侧栏展开到全局再返回，Conversation ID 与草稿保持', async ({ page }) => {
    ...
    await expect(page).toHaveURL(new RegExp(`/agent/conversations/${conversationId}$`))
    await expect(page.getByRole('complementary', { name: '会话历史导航' })).toBeVisible()
    await expect(page.getByRole('button', { name: '返回业务页面' })).toBeVisible()
    await expect(page.getByRole('button', { name: '折叠历史导航' })).toBeVisible()
  ...
  test('发送后立即展开仍停留在同一 Conversation', async ({ page }) => {
    ...
    await pane.getByRole('button', { name: '进入全局模式' }).click()
    await expect(page).toHaveURL(new RegExp(`/agent/conversations/${conversationId}$`))
    await expect(page.getByText('切换中的提问')).toHaveCount(1)
  })
})
