...
客源单级结清进度展示态，由下属应收收付款节点聚合派生：未生成、待收（已生成且未收款）、部分收款、已收齐；若曾生成的应收节点均已关闭，展示为已关闭，**不**回退为未生成，亦**不**因关闭前曾有部分核销而展示为部分收款或已收齐。仅「未生成」允许触发财务生成；其余状态客源列表操作为「查看应收」（跳转本发团应收管理，并对该客源单下全部应收行——含已关闭节点——做一次高亮闪烁定位，非选中态），不可再次生成。结清进度聚合时忽略已关闭节点：仅部分路径关闭时，状态仍由剩余活跃节点派生（待收/部分收款/已收齐），不因单条关闭整单变为已关闭。已关闭时客源单不可删除、金额字段只读，非金额字段可改。
_Avoid_: 已生成（作待收的正式 label）, 关闭后等同未生成, 用节点级结清文案直接当客源单状态枚举名, 重新生成, 关闭后可删源事实或再改金额并生成, 任一路径关闭即整单已关闭, 全部关闭且曾有核销改展示为部分收款/已收齐

**Segment Resource Payable Status（资源应付状态）**:
行程段资源级结清进度展示态，由该资源关联的应付收付款节点派生：未生成、待付（已生成且未付款）、部分付款、已付清；撤销最后一笔有效核销且节点未关闭时，从部分付款/已付清回到待付，仍视为已生成并沿用原节点继续登记付款或匹配流水。若曾生成的应付节点已关闭，展示为已关闭，**不**回退为未生成，亦**不**因关闭前曾有部分核销而展示为部分付款或已付清。规则与客源单应收状态对称：仅「未生成」可生成应付；其余状态资源列表操作为「查看应付」（跳转本发团应付管理并对该资源对应应付行高亮闪烁），不可再次生成，亦无「重新生成」。已关闭时资源不可删除、金额字段只读，非金额字段可改。
_Avoid_: 已生成（作待付的正式 label）, 撤销核销后回退未生成, 撤销核销后要求新增资源, 关闭后等同未生成, 重新生成, 与客源应收规则分叉, 关闭后可删资源或再改金额并生成, 全部关闭且曾有核销改展示为部分付款/已付清
...
团详情应收/应付用 Cascader 按 **往来对象类型 → 具体往来对象** 筛选（类型按方向裁剪：应收为合作伙伴/游客代收，应付为供应商/合作伙伴）；`changeOnSelect` 下只选类型也生效。具体对象选项来自本团该方向已出现节点去重（有 id 按 type+id，无 id 按 type+name）。列表 API 支持 `counterpartyType` / `counterpartyId` / `counterpartyName`（全局 API 同样支持，全局页 UI 暂不暴露）。节点编号/标题、节点状态、到期日仍为客户端筛选。
_Avoid_: 全局应收/应付页半成品往来对象筛选, 团内选项拉全公司档案,_Avoid_: 四按钮平铺, 查看核销作独立详情页（第一版）, 查看核销按 paymentScheduleId 深链, 已结清仍露出关闭节点, 按历史含已撤销核销记录决定入口, 已关闭一律露出查看核销, 归档发团仍露出调整约定金额, 无财务履历仍露出调整约定金额, 有效核销未清零仍露出调整约定金额, 仅应付可调而应收不可调
