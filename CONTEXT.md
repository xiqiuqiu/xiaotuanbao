...
**财务**:
负责应收、应付、财务流水、核销等财务相关菜单。重新打开已结清发团下的收付款节点时，财务可在明确确认联动影响后直接使发团回到待结算，无需 OP 预先审批；操作完整留痕并对 OP 可见，之后仍由 OP 重新确认已结清。
_Avoid_: 会计, 出纳
...
解除财务流水与收付款节点之间的有效金额分配：原流水仍是已发生的资金事实，其金额回到未核销可用余额；原节点已核销金额下降、未结清金额上升。撤销核销不作废流水、不表示资金未发生或已退回；流水本身错误应另走作废或反向资金记录。
...
财务人员从 **财务流水列表** 从某条流水发起核销的操作，与「匹配流水」互为镜像：匹配流水从收付款节点出发选流水；去核销从流水出发选收付款节点。收入流水自动进入应收核销候选；支出流水进入应付核销候选。第一版用独立抽屉 `VerifyFromTransactionDrawer`（布局镜像 `MatchTransactionDrawer`），提交复用 `createVerification`。「查看核销」跳转核销列表，经顶部筛选区按 **流水号**（`transactionNo`）过滤，不按实体 ID、不另开临时筛选条。
