...
**Counterparty（往来对象）**:
收付款节点与收支流水上标识资金往来一方的字段，存储为 `counterpartyType` + `counterpartyId` / `counterpartyName`。产品 UI 统一 label **往来对象**、**往来对象类型**（不用「交易对象」）。类型枚举：合作伙伴（Partner）、供应商（Supplier）、游客代收（Guest，存储值仍为 `guest`）、手工录入（Manual）。新建/编辑流水时：合作伙伴、供应商须从目录档案选择（写入 `counterpartyId`）；游客代收须先选关联发团再选客源单（仅列出该发团下 `guestCollectCents > 0`、即存在游客代收路径的客源单；`counterpartyId` = 客源单 id），名称可手改（如客人姓名）仅作展示；手工录入可手输名称。收支方向联动默认类型：收入 → 合作伙伴；支出 → 供应商。新建流水关联发团必填，表单中靠前填写。
_Avoid_: 交易对象, 客户/供应商/其他（作流水表单枚举 label，与 Partner/Guest 等领域类型混用）, 流水表单仅手输名称不接档案（第一版）, 客人流水无客源单 id 仍可核销游客代收, 新建流水关联发团可选, 往来类型 UI 仍称「客人」（与应收节点「游客代收」不一致）
