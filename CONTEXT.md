...
已有财务履历的源事实应付，不因有效核销归零而恢复普通金额编辑；节点未关闭且当前有效核销为零时，可通过明确调整动作同步修正原资源约定金额与原应付节点，不新建节点。调整原因必填并记录调整前后金额与操作人；仍有有效核销须先撤销，已关闭节点不可调整。
_Avoid_: 撤销核销后恢复普通金额编辑, 永久锁死无纠错入口, 调整时新建应付节点, 资源与应付节点金额分叉, 无原因调整
...
全局应收/应付列表操作列：可结算时外露「登记收款/登记付款」与「匹配流水」；「编辑」「关闭节点」「重新打开」「调整约定金额」「查看核销」收进「更多」。关闭节点仅未结清金额大于零时出现（含部分核销）；已结清与已关闭不出现关闭入口。「重新打开」仅已关闭节点出现。「调整约定金额」仅资源应付、节点开放、有效已核销为零且存在财务履历时出现；发团已归档时不出现。「查看核销」仅当有效已核销金额大于零时出现（含已关闭节点）；核销全部撤销后入口消失，审计可在核销列表按节点编号检索（含已撤销）。查看核销跳转核销列表，经顶部筛选区按 **节点编号**（`scheduleNo`）过滤，第一版不做独立核销详情抽屉。
_Avoid_: 四按钮平铺, 查看核销作独立详情页（第一版）, 查看核销按 paymentScheduleId 深链, 已结清仍露出关闭节点, 按历史含已撤销核销记录决定入口, 已关闭一律露出查看核销, 归档发团仍露出调整约定金额, 无财务履历仍露出调整约定金额, 有效核销未清零仍露出调整约定金额
