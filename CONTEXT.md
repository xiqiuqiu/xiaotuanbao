...
客源单级结清进度展示态，由下属应收收付款节点聚合派生：未生成、待收（已生成且未收款）、部分收款、已收齐；若曾生成的应收节点均已关闭，展示为已关闭，**不**回退为未生成。仅「未生成」允许触发财务生成；其余状态客源列表操作为「查看应收」（跳转本发团应收管理，并对该客源单下全部应收行——含已关闭节点——做一次高亮闪烁定位，非选中态），不可再次生成。结清进度聚合时忽略已关闭节点：仅部分路径关闭时，状态仍由剩余活跃节点派生（待收/部分收款/已收齐），不因单条关闭整单变为已关闭。已关闭时客源单不可删除、金额字段只读，非金额字段可改。
_Avoid_: 已生成（作待收的正式 label）, 关闭后等同未生成, 用节点级结清文案直接当客源单状态枚举名, 重新生成, 关闭后可删源事实或再改金额并生成, 任一路径关闭即整单已关闭

**Segment Resource Payable Status（资源应付状态）**:
行程段资源级结清进度展示态，由该资源关联的应付收付款节点派生：未生成、待付（已生成且未付款）、部分付款、已付清；若曾生成的应付节点已关闭，展示为已关闭，**不**回退为未生成。规则与客源单应收状态对称：仅「未生成」可生成应付；其余状态资源列表操作为「查看应付」（跳转本发团应付管理并对该资源对应应付行高亮闪烁），不可再次生成，亦无「重新生成」。已关闭时资源不可删除、金额字段只读，非金额字段可改。
_Avoid_: 已生成（作待付的正式 label）, 关闭后等同未生成, 重新生成, 与客源应收规则分叉, 关闭后可删资源或再改金额并生成
...
全局应收/应付列表操作列：可结算时外露「登记收款/登记付款」与「匹配流水」；「编辑」「关闭节点」「查看核销」收进「更多」。查看核销跳转核销列表，经顶部筛选区按 **节点编号**（`scheduleNo`）过滤，第一版不做独立核销详情抽屉。
_Avoid_: 四按钮平铺, 查看核销作独立详情页（第一版）, 查看核销按 paymentScheduleId 深链
