...
全局应收/应付列表操作列：可结算时外露「登记收款/登记付款」与「匹配流水」；「编辑」「关闭节点」「查看核销」收进「更多」。关闭节点仅未结清金额大于零时出现（含部分核销）；已结清与已关闭不出现关闭入口。「查看核销」仅当有效已核销金额大于零时出现（含已关闭节点）；核销全部撤销后入口消失，审计可在核销列表按节点编号检索（含已撤销）。查看核销跳转核销列表，经顶部筛选区按 **节点编号**（`scheduleNo`）过滤，第一版不做独立核销详情抽屉。
_Avoid_: 四按钮平铺, 查看核销作独立详情页（第一版）, 查看核销按 paymentScheduleId 深链, 已结清仍露出关闭节点, 按历史含已撤销核销记录决定入口, 已关闭一律露出查看核销
