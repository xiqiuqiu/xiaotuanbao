...
- **Finance presence 查询（2026-08）**：Facade 另提供 `getSourceOrderFinancePresence(s)` / `getResourceFinancePresence(s)`，返回 `{ blocksRemoval, isGenerated }`（单条 + 批量 ids；资源键为 `sourceType + sourceId`）。`blocksRemoval`：存在任一收付款节点（含作废）则禁止删除客源单/资源；应收 `isGenerated`：存在任一客源应收路径节点；应付 `isGenerated`：存在未作废资源应付节点（作废后可再进待提交）。Departure 的客源/资源 `remove`、应收/应付 gap、段内批量生成候选经此 seam，不再直连 Payment Schedule 判断「是否已生成 / 是否可删」。发团 purge 的 finance-touched（节点 ∪ 流水）本轮仍可直读，不经 presence。实现细节在 Facade 背后的内部 module，测试 surface 仍是 Facade。
