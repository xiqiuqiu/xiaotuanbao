...
开发期 `?variant=` 与独立原型沙盒已按 [#243](https://github.com/xiqiuqiu/xiaotuanbao/issues/243) 从主站移除（历史仅保留上述 PR/分支指针）。
...
8. **原型处置（已完成，#243）**：主站已移除独立沙盒路由、PrototypeSwitcher 与布局 `?variant=` 挂载；`package.json` 无 `prototype:departure-detail-layout` 生产脚本。PR #236 / 分支 `cursor/departure-detail-layout-prototype-c893` 仅作历史 primary source 指针，勿把 `prototype/**` 当生产依赖。  
